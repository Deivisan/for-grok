// full-grok-share-capture.js
// Integrated BrowserOS capture script (scroll + clean extract + Markdown).
// This is the current best draft for the reusable skill.
//
// Usage from BrowserOS:
//   1. Make sure the Grok share page is the active/target page.
//   2. browseros_evaluate_script({ page: <ID>, expression: "<paste entire content of this file>" })
//
// It will:
// - Patiently scroll until the conversation stops growing (or limits reached).
// - Run speaker-aware cleaning + Markdown assembly.
// - Return the final cleanMarkdown + diagnostics.
...
(() => {
  ...
  const CONFIG = {
    ...
    maxTotalTimeMs: 45 * 60 * 1000,   // 45 minutes max (user wants completeness over speed)
    logEveryNScrolls: 15
  };
  ...
  // Speaker detection (generic v4 heuristics)
  ...
  async function runFullCapture() {
    ...
    const result = {
      success: true,
      extractorVersion: 'full-capture-v1',
      url: location.href,
